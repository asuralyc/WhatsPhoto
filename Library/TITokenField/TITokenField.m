...
- (void)updateContentSize {
	[self setContentSize:CGSizeMake(self.bounds.size.width, CGRectGetMaxY(_tokenField.frame))];
}
...
- (void)addTokensWithTitleArray:(NSArray *)titleArray animated:(BOOL)animated {
    if (animated) {
        for (NSString *title in titleArray) {
            [self addTokenWithTitle:title];
        }
...
- (CGFloat)calculateHeight {
    return [self layoutTokensInternal];
}
...
- (void)tokenTouchUpInside:(TIToken *)token {
	if (_editable) {
		...
		if ([delegate respondsToSelector:@selector(tokenField:touchUpInsideToken:)]){
			[delegate tokenField:self touchUpInsideToken:token];
		}
    }
}
...
//	CGFloat hPadding = 8;
    CGFloat leftMargin = self.leftViewWidth + 4;// add by sapp
	CGFloat hPadding = 4;// add by sapp
...
- (void)setPlaceholder:(NSString *)placeholder {
	...
	if (placeholder){
		...
		if (!label || ![label isKindOfClass:[UILabel class]]){
			...
			[label setTextColor:[UIColor colorWithWhite:0.80 alpha:1]];
